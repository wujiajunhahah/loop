...
import type { AgentPresentation, Memory, RecipientChoice, RecipientSession } from '../../domain'
import { agentService, contextCaptureService, hardwareBridge, playbackService, relationshipStore } from '../../data/services'
...
export function RecipientExperience() {
  ...
  const [presentation, setPresentation] = useState<AgentPresentation>()
  ...
  useEffect(() => hardwareBridge.subscribe((event) => {
    if (event.type === 'touch' && (!event.actorId || event.actorId === demoRecipient.id)) {
      go('/recipient/verify')
    }
  }), [])
  ...
  useEffect(() => {
    ...
    setLoading(true)
    void (async () => {
      const memories = await relationshipStore.getMemoriesForRecipient(
      ...
      const policy = {
        ...
        allowNewMemoryGeneration: false as const,
        allowedMemoryIds: memories.map((item) => item.id),
        blockedTopics: [],
        proactiveDelivery: 'after_recipient_entry' as const,
      }
      const composed = await agentService.compose({
        relationshipId: demoRecipient.relationshipId,
        recipientId: demoRecipient.id,
        policy,
      })
      const selected = composed?.memoryIds[0]
        ? memories.find((item) => item.id === composed.memoryIds[0])
        : undefined
      if (!cancelled) {
